...
\subsection{Selecting features to track}
...
\section{Introduction}
As described in previous sections, the amount of data furnished by sensors can be huge to handle, thus it becomes important to extract only the relevant part of it.
The \emph{features} are a good representation of main content inside data and are easy to manage; it is possible to design a model for them that keeps into account the presence of noise derived from sensors' measurements. 

In this chapter, we will deal with the choice of lines as features and a possible representation for them, based on normals and middlepoints. Such representation can be exploited to design a least square estimation problem, whose goal is finding a transformation $T$ allowing to align two different robot's scans, through an iterative process that uses associations detected between lines belonging to scans themselves.
...
\begin{equation}
	f(z_t) = \{f_t^1, f_t^2, \dots \} =\left \{ \left( \begin{matrix} r_t^1 \\ \phi_t^1 \\ s_t^1 \end{matrix} \right),  \left(\begin{matrix} r_t^2 \\ \phi_t^2 \\ s_t^2 \end{matrix} \right), \dots \right\}
\end{equation}
...
\begin{itemize}
	...
	\item{\emph{Step 2}: associate each transformed line to the closest line in scan $i$; such line is sought first by evaluating the normals of lines in $i$ and constituting an initial pool of possible associations.hen, the lines in the pool are evaluated on the basis of their $\theta$, to decide the final association}
